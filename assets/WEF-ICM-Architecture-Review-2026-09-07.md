...
**Status: RESOLVED (2026-09-07).** All eight findings are addressed and merged — **F-1, F-2, F-3, F-5, F-6, F-7 → CR-027**; **F-4 → CR-028**; **F-8 → CR-029** (Board chose Option C: split + pointer stub + deferred opportunistic reference sweep, the sweep tracked in [`CR-029-Reference-Sweep-Checklist.md`](CR-029-Reference-Sweep-Checklist.md)); the `master-content-workbook` skill named in F-3 → **CR-030**. Canonical records: [`../WEF-v1.0/_change-requests/`](../WEF-v1.0/_change-requests/) CR-027 – CR-030. The original proposal drafts and the Board memo are archived in [`_archive/`](_archive/). This document is retained as the audit of record. A second pass ([`WEF-ICM-Cleanup-Pass-2-2026-09-07.md`](WEF-ICM-Cleanup-Pass-2-2026-09-07.md)) tracks follow-on cleanup (C-1 … C-8).
...
| 7 | Move CR bodies to `WEF-v1.0/_change-requests/CR-0NN.md`; thin the Revision Log | F-4 | **CR-028** | **merged** |
| 8 | Split `01-Governance.md` into a `01-governance/` sub-folder with its own router | F-8 | **CR-029** | **merged** — Board Option C; opportunistic reference sweep tracked in `CR-029-Reference-Sweep-Checklist.md` |

All eight items are merged (CR-027 – CR-030). Every change was additive or reconciling — no Stage Gate exit criteria, persona library, compliance landscape, Eight-Dimension standard, default technology stack, or prior approval was affected. The one open thread is the CR-029 opportunistic Governance reference sweep, tracked separately.
