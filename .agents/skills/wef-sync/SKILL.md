...
Request per sync pass — never as duplicate, independently-authored branches. See RETRO-017
(Governance, Sec. 15.4) for the actual incident this exists to prevent: three branches carrying
overlapping content for what was ultimately a single Change Request, followed immediately by a
second collision — this pipeline's own CR/RETRO numbers collided with a concurrent engagement's
...
renumbered to CR-022/RETRO-017 (Governance, Sec. 15.4) — reservation lowers the odds, it doesn't
guarantee uniqueness on its own.

## Step 5 — Draft the methodology change

For each finding in the batch, write it up the way RETRO-001 through RETRO-017 are written
...
practice. **Before merging, re-fetch and re-check Step 4's reservation is still yours** — if a
concurrent session's PR merged first and consumed the same CR number, renumber before merging
rather than after (recovering post-merge, as this pipeline itself had to, costs a full rebase and
history-narrating cleanup). On merge, flip every source engagement's `WEF-Candidate-Findings.md`
row from `Flagged` to `Merged` with the CR ID. This closing step is why Step 2's per-finding
source tracking matters — without it, there's no way back to the originating file to close the
loop.
