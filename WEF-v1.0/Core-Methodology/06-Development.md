...
| Configure GA4/GSC/GTM/Clarity/WPConsent | Unchanged — these are platform-independent and apply regardless of the confirmed stack |
...
**Verify every AI-driven CMS/plugin edit via a genuine fresh page reload, not by reading the in-session DOM state, and never write into a plugin's client-side state-management store directly.** Two distinct, confirmed failure patterns, both requiring a fresh navigation/reload to catch: (1) a scripted edit (native-setter + dispatched `input`/`change` event on the real, visible form control) can *look* successful in the same browser session while silently failing to persist; (2) calling a plugin's own Redux/state-management store update API directly (e.g., a Gutenberg SEO plugin's `wp.data.dispatch('plugin-slug').updateX(...)`) is a *distinct and more deceptive* failure mode, not an "anti-tampering guard" — it updates every place that reads from the store (visible previews, even live-computed derived scores) while never touching the dirty-field tracking the save action actually reads, so the edit is silently dropped on save with no in-session signal that anything is wrong (Governance, Sec. 15.4, RETRO-019). The fix for both is the same and is now the standing default, not a fallback reached for after the store-API approach fails: locate the real, visible form control for the field and drive the edit through it — native property setter plus dispatched `input`/`change` events — using the store API only to *read* current state, never to write it. If a field still doesn't persist after that genuine technique and a fresh reload, check whether a different, underlying WordPress-native field feeds the same displayed value via a template token (e.g., editing the post's actual title, which a plugin's "%title%" token then inherits), and edit that instead of continuing to fight the plugin's own UI layer.

**Treat a plugin's bulk/batch recalculation tool as an approximation, not ground truth, whenever a live per-item equivalent exists.** A confirmed real-world case: an SEO plugin's bulk "Recalculate Scores" tool made several pages' scores *worse*, including one dropping by double digits, for content that had only improved. Opening the same page in the live editor — where the plugin's real JS analyzer runs against fully-rendered content — showed a meaningfully higher, correct score for identical content; saving from there (via the real-input-event technique above) persisted the accurate number (Governance, Sec. 15.4, RETRO-020). Bulk/batch variants of a computed value commonly run in a reduced server-side context and are not guaranteed to match the live computation's accuracy. Verify at least one sample from any bulk-tool run against the live per-item path before trusting it, and for anything but a very large page count, prefer scripting the live-editor path (open, let it compute, save) over a bulk shortcut.
