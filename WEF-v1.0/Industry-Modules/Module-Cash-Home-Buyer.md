...
**Adjacent, distinct business model (documented gap, not yet a full Module): the licensed referral intermediary.** A confirmed real-world variant is a licensed real estate broker who is neither the cash buyer (this Module's core profile) nor the listing brokerage (Real Estate/Distressed Property Advocate Modules), but instead refers a seller to *either* a vetted investor network *or* an independent licensed brokerage, earning referral compensation from whichever path closes — never purchasing directly and never acting as listing agent. This is close enough to blend with this Module (as the investor-path side) plus the Distressed Property Advocate Module (as the brokerage-referral side), but its compliance posture is distinct: every page must carry a standing disclosure that the operator is *not necessarily* the purchaser or listing brokerage, and that compensation may flow from either referral path without affecting the terms offered to the seller. If a firm takes on this exact business model repeatedly, it warrants its own Module authored via the New Module Development Process (Governance, Sec. 13.6); until then, treat it as a documented blend with an added standing disclosure requirement, not a clean fit into either existing Module alone.
...
| "How We Evaluate/Vet [Investors/Partners]" Page *(confirmed real-world pattern, referral-intermediary variant)* | Publishing the operator's own vetting criteria for any third party (investor, brokerage) a seller may be connected to — proof of funds, track record review, transparent-terms requirement, no-pressure-tactics standard. A genuine trust-building pattern for any referral-based variant of this Module, not just a nice-to-have |
| "Licensing, Relationships & Compensation" Page *(confirmed real-world pattern, referral-intermediary variant)* | A dedicated transparency page distinct from About — plainly answers who the operator is, whether they're the buyer, whether they're the listing brokerage, and exactly how they're compensated (referral fees, investor-related compensation). Directly addresses the "who am I actually dealing with" objection at the root of this vertical's predatory-practice reputation risk (Section 2) |
